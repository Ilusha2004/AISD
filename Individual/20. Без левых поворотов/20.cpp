...
void DFS(long long start, long long start_intersection, long long finish_intersection) {
    ...
    int i = 1;
    std::for_each(++intersection_coodrinates.begin(), intersection_coodrinates.end(), [&i, &start, start_intersection, &finish_intersection](std::pair<long long, long long>& temp_intersection){
...
int main()
{
    ...
    intersection_coodrinates.resize(count_intersection + 3);
    matrix.assign(count_intersection + 3, std::vector<std::pair<long long, bool> >(count_intersection + 3));
